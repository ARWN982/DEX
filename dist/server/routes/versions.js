...
// Paths to data files
const VERSIONS_PATH = path_1.default.join(__dirname, '../../../src/data/versions.json');
// Helper function to scan filesystem for actual version folders for a specific page (local dev only)
async function getVersionsFromFilesystem(pageName = 'simple-esql') {
    try {
        console.log(`Scanning filesystem for versions of page: ${pageName}`);
        // Scan the src/public/pages directory (development)
        const pageDir = path_1.default.join(__dirname, `../../../src/public/pages/${pageName}`);
        const versionFolders = await fs_1.promises.readdir(pageDir);
...
router.get('/', async (req, res) => {
    try {
        const pageName = req.query.page || 'simple-esql';
        ...
        try {
            const data = await fs_1.promises.readFile(VERSIONS_PATH, 'utf8');
...
router.post('/', async (req, res) => {
    try {
        ...
        await fs_1.promises.writeFile(VERSIONS_PATH, JSON.stringify(versionsData, null, 2));
...
async function createVersionFiles(versionId, baseVersionId, pageName) {
    const targetPage = pageName || 'simple-esql';
    // Try dist/public/pages first (production), fall back to src/public/pages (development)
    let pagesDir = path_1.default.join(__dirname, '../../public/pages');
    try {
        await fs_1.promises.access(pagesDir);
    }
    catch {
        pagesDir = path_1.default.join(__dirname, '../../../src/public/pages');
    }
    const versionDir = path_1.default.join(pagesDir, targetPage, `v${versionId}`);
    try {
        // Ensure version directory exists
        await fs_1.promises.mkdir(versionDir, { recursive: true });
        const commentsFile = path_1.default.join(versionDir, 'comments.json');
        const jobStoriesFile = path_1.default.join(versionDir, 'jobStories.json');
        ...
        if (baseVersionId) {
            try {
                const baseVersionDir = path_1.default.join(pagesDir, targetPage, `v${baseVersionId}`);
                const baseCommentsFile = path_1.default.join(baseVersionDir, 'comments.json');
                const baseCommentsData = await fs_1.promises.readFile(baseCommentsFile, 'utf8');
        ...
        await fs_1.promises.writeFile(commentsFile, JSON.stringify(commentsData, null, 2));
        ...
        if (baseVersionId) {
            try {
                const baseVersionDir = path_1.default.join(pagesDir, targetPage, `v${baseVersionId}`);
                const baseJobStoriesFile = path_1.default.join(baseVersionDir, 'jobStories.json');
                const baseJobStoriesData = await fs_1.promises.readFile(baseJobStoriesFile, 'utf8');
        ...
        await fs_1.promises.writeFile(jobStoriesFile, JSON.stringify(jobStoriesData, null, 2));
...
async function createEmptyStateTemplate(filePath, pageName, versionId) {
    try {
...
`;
        await fs_1.promises.writeFile(filePath, template, 'utf8');
...
async function createVersionComponentFiles(versionId, baseVersionId, pageName) {
    // Try dist/public/pages first (production), fall back to src/public/pages (development)
    let pagesDir = path_1.default.join(__dirname, '../../public/pages');
    try {
        await fs_1.promises.access(pagesDir);
    ...
    try {
        for (const pageName of pageNames) {
            const versionDir = path_1.default.join(pagesDir, pageName, `v${versionId}`);
            const componentFile = path_1.default.join(versionDir, `index.tsx`);
            ...
            if (baseVersionId) {
                ...
                const baseVersionDir = path_1.default.join(pagesDir, pageName, `v${baseVersionId}`);
                const baseComponentFile = path_1.default.join(baseVersionDir, `index.tsx`);
                try {
                    await fs_1.promises.access(baseComponentFile);
                    sourceFile = baseComponentFile;
                }
                catch {
                    // Fallback to main component if base version doesn't exist
                    sourceFile = path_1.default.join(pagesDir, `index.tsx`);
                }
            }
            else {
            ...
            try {
                ...
                // Copy the file
                await fs_1.promises.copyFile(sourceFile, componentFile);
...
async function updateImportPaths(filePath, pageName) {
    try {
        ...
        if (updatedContent !== content) {
            await fs_1.promises.writeFile(filePath, updatedContent, 'utf8');
