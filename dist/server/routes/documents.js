...
function generateFallbackData(index) {
...
const documentsHandler = async (req, res) => {
    ...
    const { index = 'remote_cluster:logs*', search = '', from = 'now-15h', // Changed from 'now-15m' to 'now-15h' to match frontend default
    to = 'now', size = '1000', filters = [] } = params;
    try {
        ...
        const response = await Promise.race([
            elasticsearch_1.esClient.search({
                index: index,
                body: searchBody
            }),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Search timeout after 25s')), 25000) // 25 second timeout
            )
        ]);
        // Format documents for the frontend
        const responseBody = response.body || response;
        const hits = responseBody.hits;
        const documents = hits.hits.map((hit) => ({
            _id: hit._id,
            _index: hit._index,
            ...hit._source
        }));
        // Analyze timestamp distribution of returned documents
        const timestamps = documents.map((doc) => doc['@timestamp']).filter((ts) => ts).sort();
        ...
        const mockDocuments = generateFallbackData(index);
        res.status(200).json({
