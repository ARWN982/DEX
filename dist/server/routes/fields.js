...
router.get("/fields/:index", async (req, res) => {
    const { index } = req.params;
    try {
        // Use Elasticsearch field_caps API to get comprehensive field information
        const response = await Promise.race([
            elasticsearch_1.esClient.fieldCaps({
                index: index,
                fields: "*", // Get all fields
                include_unmapped: false // Only include mapped fields
            }),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Field capabilities timeout after 20s')), 20000))
        ]);
        const body = response.body || response;
        const fields = body.fields || {};
        // Transform the response into a more usable format
        const fieldInfo = Object.entries(fields).map(([fieldName, fieldData]) => {
            // Get the field type from the first mapping (most common)
            const firstMapping = Object.values(fieldData)[0];
