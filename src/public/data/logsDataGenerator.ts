...
export class LogsDataGenerator implements DataGenerator<LogDocument> {
  // In-memory cache for generated logs
  private static cachedLogs: LogDocument[] | null = null;
  
  // Field type mapping - based on real Elasticsearch log structure (378+ fields)
  private fieldTypeMap: Record<string, FieldType> = {
    _id: 'keyword',
    _index: 'keyword',
    _version: 'number',
    _score: 'number',
  ...
  async generateData(params: DataGeneratorParams): Promise<LogDocument[]> {
    ...
    if (params.from && params.to) {
      ...
      const fromMoment = dateMath.parse(params.from);
      const toMoment = dateMath.parse(params.to, { roundUp: true });
  ...
  getAvailableFields(data: LogDocument[]): string[] {
    ...
    if (data.length > 0) {
      ...
      const hasTypicalLogFields = docKeys.some(key => 
        key === '@timestamp' || 
        key === 'message' || 
        key.startsWith('log.') || 
        key.startsWith('host.') ||
  ...
  private static cachedData: LogDocument[] | null = null;
  ...
  private generateSampleLogData(): LogDocument[] {
    ...
    const cloudProviders = ["gcp", "aws", "azure"] as const;
    const cloudPlatforms = ["gcp_kubernetes_engine", "aws_eks", "azure_aks"];
    const availabilityZones: Record<string, string[]> = {
    ...
    const data = Array(10000)
      ...
      .map((_, index) => {
        ...
        const hostIps = Array.from({ length: Math.floor(seededRandom() * 6) + 1 }, () => 
          `10.${Math.floor(seededRandom() * 255)}.${Math.floor(seededRandom() * 255)}.${Math.floor(seededRandom() * 255)}`
        );
        const hostMacs = Array.from({ length: Math.floor(seededRandom() * 6) + 1 }, () => 
          `${Math.floor(seededRandom() * 256).toString(16).padStart(2, '0')}-${Math.floor(seededRandom() * 256).toString(16).padStart(2, '0')}-${Math.floor(seededRandom() * 256).toString(16).padStart(2, '0')}-${Math.floor(seededRandom() * 256).toString(16).padStart(2, '0')}-${Math.floor(seededRandom() * 256).toString(16).padStart(2, '0')}-${Math.floor(seededRandom() * 256).toString(16).padStart(2, '0')}`
        );
