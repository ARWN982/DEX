...
// Data source types
export const dataSourceTypes = ["Integration", "Stream", "Index"] as const;
 ...
 */
export function getDataSources(): DataSourceOption[] {
 ...
 */
export function getDataSourcesByType(types: DataSourceType[]): DataSourceOption[] {
  if (types.length === 0) return dataSourceOptions;
  return dataSourceOptions.filter(option => types.includes(option.type));
}
 ...
 * @returns Filtered array of data source options
 */
export function getDataSourcesBySearch(searchTerm: string): DataSourceOption[] {
  if (!searchTerm.trim()) return dataSourceOptions;
 ...
 * @returns Filtered array of data source options
 */
export function filterDataSources(types: DataSourceType[], searchTerm: string): DataSourceOption[] {
  let filtered = dataSourceOptions;
 ...
 */
export function createLabelToIndexMapping(
  customMappings: Record<string, string> = {}
): Record<string, string> {
  const defaultMappings = Object.fromEntries(
    dataSourceOptions.map(option => [option.label, option.label.toLowerCase()])
  );
 ...
 */
export function createIndexToLabelMapping(
  labelToIndexMapping: Record<string, string>
): Record<string, string> {
  return Object.fromEntries(
    Object.entries(labelToIndexMapping).map(([label, index]) => [index, label])
  );
}
 ...
 */
export function getDataSourcesByCategory(): Record<DataSourceType, DataSourceOption[]> {
  const grouped = dataSourceOptions.reduce((acc, option) => {
    if (!acc[option.type]) {
      acc[option.type] = [];
    }
 ...
 */
export function getDataSourceCounts(): Record<DataSourceType, number> {
  const counts = dataSourceOptions.reduce((acc, option) => {
